...
void ELFINAATRCOutput::init(void* machtnichts) {}

void ELFINAATRCOutput::set_freq(uint32_t chmask, uint16_t freq_hz) 
{
    for (int i = 0; i < 32; i++) {
        if ((chmask >> i) & 1) {
            _set_freq(i, freq_hz);
...
uint16_t ELFINAATRCOutput::get_freq(uint8_t ch) 
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return 0;
...
void ELFINAATRCOutput::enable_ch(uint8_t ch)
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return;
    uint8_t pin = _channel_to_flymaple_pin(ch);
...
void ELFINAATRCOutput::disable_ch(uint8_t ch)
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return;
    uint8_t pin = _channel_to_flymaple_pin(ch);
...
void ELFINAATRCOutput::write(uint8_t ch, uint16_t period_us)
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return;
    uint8_t pin = _channel_to_flymaple_pin(ch);
...
void ELFINAATRCOutput::write(uint8_t ch, uint16_t* period_us, uint8_t len)
{
    for (int i = 0; i < len; i++)
        write(i + ch, period_us[i]); 
}

uint16_t ELFINAATRCOutput::read(uint8_t ch) 
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return 0;
    uint8_t pin = _channel_to_flymaple_pin(ch);
...
void ELFINAATRCOutput::read(uint16_t* period_us, uint8_t len)
{
    for (int i = 0; i < len; i++)
        period_us[i] = read(i);
}

uint8_t ELFINAATRCOutput::_channel_to_flymaple_pin(uint8_t ch)
{
    // This maps the ArduPilot channel numbers to ElfinAAT PWM output pins
    ...
    static uint8_t ch_to_pin[ELFINAAT_RC_OUTPUT_NUM_CHANNELS] = { 28, 27, 11, 12, 24, 14 };
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
...
void ELFINAATRCOutput::_set_freq(uint8_t ch, uint16_t freq_hz) 
{
    if (ch >= ELFINAAT_RC_OUTPUT_NUM_CHANNELS)
	return;
    if (freq_hz == 0)
