...
ELFINAATUARTDriver::ELFINAATUARTDriver(HardwareSerial* hws):
    _hws(hws),
    _txBuf(NULL),
...
void ELFINAATUARTDriver::begin(uint32_t b) 
{
...
void ELFINAATUARTDriver::begin(uint32_t b, uint16_t rxS, uint16_t txS) 
{
...
void ELFINAATUARTDriver::end() 
{
    _hws->end();
}

void ELFINAATUARTDriver::flush() 
{
    _hws->flush();
}

bool ELFINAATUARTDriver::is_initialized() 
{ 
    return true; 
}

void ELFINAATUARTDriver::set_blocking_writes(bool blocking) {}

bool ELFINAATUARTDriver::tx_pending() { return false; }

/* ELFINAAT implementations of Stream virtual methods */
int16_t ELFINAATUARTDriver::available() 
{ 
    return _hws->available(); 
}

int16_t ELFINAATUARTDriver::txspace() 
{ 
    // Mikems fork of libmaple includes usart TX buffering
    return _hws->c_dev()->tx_rb->size - rb_full_count(_hws->c_dev()->tx_rb);
}

int16_t ELFINAATUARTDriver::read() 
{ 
    return _hws->read(); 
}

/* ELFINAAT implementations of Print virtual methods */
size_t ELFINAATUARTDriver::write(uint8_t c) 
{
    _hws->write(c); 
    return 1; 
}

size_t ELFINAATUARTDriver::write(const uint8_t *buffer, size_t size)
{
    size_t n = 0;
    while (size--) {
        n += write(*buffer++);
