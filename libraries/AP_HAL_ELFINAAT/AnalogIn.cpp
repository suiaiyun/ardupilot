...
ELFINAATAnalogIn::ELFINAATAnalogIn() :
    _vcc(ELFINAATAnalogSource(ANALOG_INPUT_BOARD_VCC))
{}

void ELFINAATAnalogIn::init(void* machtnichts) {
    /* Register ELFINAATAnalogIn::_timer_event with the scheduler. */
    hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&ELFINAATAnalogIn::_timer_event, void));
    /* Register each private channel with ELFINAATAnalogIn. */
    _register_channel(&_vcc);
}

ELFINAATAnalogSource* ELFINAATAnalogIn::_create_channel(int16_t chnum) {
    ELFINAATAnalogSource *ch = new ELFINAATAnalogSource(chnum);
    _register_channel(ch);
    return ch;
}

void ELFINAATAnalogIn::_register_channel(ELFINAATAnalogSource* ch) {
    if (_num_channels >= ELFINAAT_INPUT_MAX_CHANNELS) {
        for(;;) {
            hal.console->print_P(PSTR(
                "Error: AP_HAL_ELFINAAT::ELFINAATAnalogIn out of channels\r\n"));
...
void ELFINAATAnalogIn::_timer_event(void) 
{
    adc_reg_map *regs = ADC1->regs;
...
AP_HAL::AnalogSource* ELFINAATAnalogIn::channel(int16_t ch) 
{
...
float ELFINAATAnalogIn::board_voltage(void)
{
