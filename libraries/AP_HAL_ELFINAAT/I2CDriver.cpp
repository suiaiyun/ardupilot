...
#define ELFINAAT_I2C_DEVICE I2C1

ELFINAATI2CDriver::ELFINAATI2CDriver(AP_HAL::Semaphore* semaphore) 
    : _semaphore(semaphore),
      _timeout_ms(0) 
{
}

void ELFINAATI2CDriver::begin() 
{
    _reset();
}

void ELFINAATI2CDriver::end() {}

void ELFINAATI2CDriver::setTimeout(uint16_t ms) 
{
    _timeout_ms = ms;
}

void ELFINAATI2CDriver::setHighSpeed(bool active) {}

uint8_t ELFINAATI2CDriver::write(uint8_t addr, uint8_t len, uint8_t* data)
{
    i2c_msg msgs[1];
    msgs[0].addr = addr;
...
uint8_t ELFINAATI2CDriver::writeRegister(uint8_t addr, uint8_t reg, uint8_t val)
{
    return writeRegisters(addr, reg, 1, &val);
}

uint8_t ELFINAATI2CDriver::writeRegisters(uint8_t addr, uint8_t reg,
...
uint8_t ELFINAATI2CDriver::read(uint8_t addr, uint8_t len, uint8_t* data)
{
...
uint8_t ELFINAATI2CDriver::readRegister(uint8_t addr, uint8_t reg, uint8_t* data)
{
    return readRegisters(addr, reg, 1, data);
}

uint8_t ELFINAATI2CDriver::readRegisters(uint8_t addr, uint8_t reg,
                              uint8_t len, uint8_t* data)
{
...
uint8_t ELFINAATI2CDriver::lockup_count() {
return _lockup_count;
}

uint8_t            ELFINAATI2CDriver::_transfer(i2c_msg *msgs, uint16 num)
{
...
void ELFINAATI2CDriver::_reset()
{
    i2c_disable(ELFINAAT_I2C_DEVICE);
    i2c_master_enable(ELFINAAT_I2C_DEVICE, I2C_FAST_MODE | I2C_BUS_RESET);
}
