...
#ifndef __AP_HAL_ELFINAAT_ANALOGIN_H__
#define __AP_HAL_ELFINAAT_ANALOGIN_H__

#include "AP_HAL_ELFINAAT.h"

#define ELFINAAT_INPUT_MAX_CHANNELS 12
...
#define ELFINAAT_VCC_ANALOG_IN_PIN 15

class AP_HAL_ELFINAAT_NS::ELFINAATAnalogSource : public AP_HAL::AnalogSource {
public:
    friend class AP_HAL_ELFINAAT_NS::ELFINAATAnalogIn;

    ELFINAATAnalogSource(uint8_t pin);
    float read_average();
    float read_latest();
...
class AP_HAL_ELFINAAT_NS::ELFINAATAnalogIn : public AP_HAL::AnalogIn {
public:
    ELFINAATAnalogIn();
...
protected: 
    ELFINAATAnalogSource* _create_channel(int16_t num);
    void _register_channel(ELFINAATAnalogSource*);
    void _timer_event(void);
    ELFINAATAnalogSource* _channels[ELFINAAT_INPUT_MAX_CHANNELS];
    int16_t _num_channels;
    int16_t _active_channel;
    uint16_t _channel_repeat_count;
    
private:
    // On ElfinAAT, VCC measurement is at pin 20. VCC (=VIN) of 5V is only present
    // if external voltage (not USB) is connected. Also there is a voltage
    // divider (25k/5k) 
    ELFINAATAnalogSource _vcc;
};
#endif // __AP_HAL_ELFINAAT_ANALOGIN_H__
