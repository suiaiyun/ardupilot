...
// These are the last 2 2k pages in the FLASH ROM address space on the RET6 used by ElfinAAT
...
ELFINAATStorage::ELFINAATStorage()
{}

void ELFINAATStorage::init(void*)
{
	...
	uint16 result = eeprom[i].init(last_flash_page - (2*i * pageSize), 
				       ...
				       pageSize);
	if (result != EEPROM_OK)
	    hal.console->printf("ELFINAATStorage::init eeprom.init[%d] failed: %x\n", i, result);
    }
}

uint8_t ELFINAATStorage::read_byte(uint16_t loc){
//hal.console->printf("read_byte %d\n", loc);
    ...
    {
	hal.console->printf("ELFINAATStorage::read_byte loc %d out of range\n", loc);
...
void ELFINAATStorage::read_block(void* dst, uint16_t src, size_t n) {
...
void ELFINAATStorage::write_byte(uint16_t loc, uint8_t value)
{
//    hal.console->printf("write_byte %d, %d\n", loc, value);
    ...
    if (eeprom_index >= num_eeprom_blocks)
    {
	hal.console->printf("ELFINAATStorage::write_byte loc %d out of range\n", loc);
	return;
    }
...
void ELFINAATStorage::write_block(uint16_t loc, const void* src, size_t n)
{
