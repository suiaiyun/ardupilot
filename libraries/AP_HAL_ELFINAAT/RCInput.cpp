...
volatile uint16_t ELFINAATRCInput::_pulse_capt[ELFINAAT_RC_INPUT_NUM_CHANNELS] = {0};  
volatile uint8_t  ELFINAATRCInput::_valid_channels = 0;
volatile uint32_t ELFINAATRCInput::_last_input_interrupt_time = 0; // Last time the input interrupt ran

// Pin 6 is connected to timer 1 channel 1
#define ELFINAAT_RC_INPUT_PIN 6
...
#define ELFINAAT_TIMER_RELOAD 60000

ELFINAATRCInput::ELFINAATRCInput()
{}

// This interrupt triggers on a negative transiution of the PPM-SIM pin
void ELFINAATRCInput::_timer_capt_cb(void)
{
    ...
    timer_dev *tdev = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_device;
    uint8 timer_channel = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_channel;
    ...
    if (current_count < previous_count) {
        pulse_width = current_count + ELFINAAT_TIMER_RELOAD - previous_count;
    } else {
...
//	    hal.uartA->printf("ch 0 %d\n", pulse_width);
        ...
        if (channel_ctr < ELFINAAT_RC_INPUT_NUM_CHANNELS) {
            _pulse_capt[channel_ctr] = pulse_width;
            channel_ctr++;
            if (channel_ctr == ELFINAAT_RC_INPUT_NUM_CHANNELS) {
                _valid_channels = ELFINAAT_RC_INPUT_NUM_CHANNELS;
            }
        }
    }
...
uint16_t ELFINAATRCInput::read(uint8_t ch) {
    timer_dev *tdev = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_device;
    uint8 timer_channel = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_channel;

    /* constrain ch */
    if (ch >= ELFINAAT_RC_INPUT_NUM_CHANNELS) 
	return 0;
...
uint8_t ELFINAATRCInput::read(uint16_t* periods, uint8_t len) {
    timer_dev *tdev = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_device;
    uint8 timer_channel = PIN_MAP[ELFINAAT_RC_INPUT_PIN].timer_channel;

    /* constrain len */
    if (len > ELFINAAT_RC_INPUT_NUM_CHANNELS) 
	len = ELFINAAT_RC_INPUT_NUM_CHANNELS;
...
bool ELFINAATRCInput::set_overrides(int16_t *overrides, uint8_t len) {
    bool res = false;
    for (uint8_t i = 0; i < len; i++) {
...
bool ELFINAATRCInput::set_override(uint8_t channel, int16_t override) {
    if (override < 0) return false; /* -1: no change. */
    if (channel < ELFINAAT_RC_INPUT_NUM_CHANNELS) {
...
void ELFINAATRCInput::clear_overrides()
{
    for (uint8_t i = 0; i < ELFINAAT_RC_INPUT_NUM_CHANNELS; i++) {
        _override[i] = 0;
    }
}
