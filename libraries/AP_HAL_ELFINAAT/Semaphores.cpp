...
// Constructor
ELFINAATSemaphore::ELFINAATSemaphore() : _taken(false) {}

bool ELFINAATSemaphore::give() {
    if (!_taken) {
        return false;
    } else {
        _taken = false;
...
bool ELFINAATSemaphore::take(uint32_t timeout_ms) {
    if (hal.scheduler->in_timerprocess()) {
        hal.scheduler->panic(PSTR("PANIC: ELFINAATSemaphore::take used from "
...
bool ELFINAATSemaphore::take_nonblocking() {
    if (hal.scheduler->in_timerprocess()) {
...
bool ELFINAATSemaphore::_take_from_mainloop(uint32_t timeout_ms) {
...
bool ELFINAATSemaphore::_take_nonblocking() {
    bool result = false;
    noInterrupts();
    if (!_taken) {
        _taken = true;
